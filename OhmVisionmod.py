...
COLOUR_BOUNDS = [
                ...
                [(0, 90, 10)    , (15, 250, 130)  , "BROWN"  , 1 , (0,51,102)    ],    
                [(0, 30, 80)    , (5, 255, 200)  , "RED"    , 2 , (0,0,255)     ],
                [(10, 100, 30)   , (15, 255, 200)  , "ORANGE" , 3 , (0,128,255)   ], 
                [(30, 100, 130) , (45, 250, 255)  , "YELLOW" , 4 , (0,255,255)   ],
                [(35, 20, 110)  , (60, 45, 120)   , "GREEN"  , 5 , (0,255,0)     ],  
                [(65, 0, 85)    , (115, 100, 147)  , "BLUE"   , 6 , (255,0,0)     ],  
                [(120, 40, 100) , (140, 250, 220) , "PURPLE" , 7 , (255,0,127)   ], 
                [(0, 0, 50)     , (50, 70, 180)   , "GRAY"   , 8 , (128,128,128) ],      
...
def printResult(sortedBands, liveimg):
    #x,y,w,h = resPos
    strVal = ""
...
def findBands(resistorInfo, DEBUG):
    ...
    #enlarge image
    resImg = cv2.resize(resistorInfo, (400, 200))
    #resPos = resistorInfo[1]
    #apply bilateral filter and convert to hsv                                          
    pre_bil = cv2.bilateralFilter(resImg,9,80,80)
    hsv = cv2.cvtColor(pre_bil, cv2.COLOR_BGR2HSV)
    #edge threshold filters out background and resistor body
    thresh = cv2.adaptiveThreshold(cv2.cvtColor(pre_bil, cv2.COLOR_BGR2GRAY),255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,19,5)
    ...
    cv2.imshow("prebil",pre_bil)#shows the most recent resistor checked.
...
sortedBands = findBands(cliveimg,DEBUG)
print(printResult(sortedBands, cliveimg))
